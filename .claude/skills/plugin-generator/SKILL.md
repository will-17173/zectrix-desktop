...
插件代码在 **QuickJS** 环境中执行，这是一个轻量级 JavaScript 引擎：
- **没有原生 fetch API** — 必须使用内置的 `fetchJson`, `fetchBase64`, `postJson` 等函数
- **支持 ES2020 语法** — async/await, const/let, 箭头函数, 模板字符串等
- **不支持浏览器/Node.js API** — 没有 DOM, setTimeout, process 等

## 可用内置函数

| 函数 | 用途 |
|------|------|
| `fetchJson(url)` | GET 请求，返回 JSON 对象 |
| `fetchJsonWithHeaders(url, headers)` | GET 请求，带自定义 Headers |
| `fetchText(url)` | GET 请求，返回文本 |
| `fetchBase64(url)` | GET 图片，返回 base64 Data URL |
| `fetchBase64WithHeaders(url, headers)` | GET 图片带 Headers |
| `postJson(url, bodyStr)` | POST JSON，body 需为 JSON 字符串 |
| `postJsonWithHeaders(url, bodyStr, headers)` | POST JSON 带 Headers |
| `generateQrCode(text)` | 生成二维码 PNG |
| `sleep(ms)` | 同步等待 |
| `config` | 用户配置对象 |
