...
macOS 对未经 Apple 签名的应用会显示此提示，这是系统安全机制，应用本身没有问题。

**解决方法：** 打开终端，执行以下命令，然后再双击打开应用：

```bash
xattr -cr /Applications/Zectrix\ Desktop.app
```

> 如果应用不在 Applications 文件夹，把 `.app` 文件直接拖进终端窗口，路径会自动填入。
...
![插件市场](./assets/images/plugin-market.png)
