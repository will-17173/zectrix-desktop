...
- `6` 开头：沪市，东方财富 `secid` 前缀 `1.`
- `0` 或 `3` 开头：深市，东方财富 `secid` 前缀 `0.`
- 其他开头：不支持，添加时提示“仅支持 0、3、6 开头的 A 股代码”

行情客户端使用东方财富实时行情接口。示例：

```text
https://push2.eastmoney.com/api/qt/ulist.np/get?fltt=2&invt=2&fields=f12,f14,f2,f3,f4&secids=1.600519,0.000001
```

请求时后端设置常见浏览器 `User-Agent` 和东方财富 `Referer`。接口返回 UTF-8 JSON，普通 A 股使用以下字段：
...
公开接口不可用、返回格式异常、缺少关键字段、价格无法解析，或返回记录数量少于本地股票列表时，本次推送失败并返回明确错误。

格式化逻辑集中在后端，生成最终自由排版文本：

1. 第一行是本次行情获取时间，使用后端完成行情请求后的本地时间，格式为 `YYYY-MM-DD HH:mm:ss`。
2. 第二行为空行。
3. 后续每只股票一行，顺序与本地股票列表一致。
4. 涨跌值和涨跌百分比使用接口返回值并格式化为两位小数，上涨补 `+`，下跌保留 `-`。
...
- 股票代码市场推断：`600519 -> 1.600519`，`000001 -> 0.000001`，`300750 -> 0.300750`。
- 非法代码被拒绝。
- 推送文本格式包含唯一更新时间和股票名称。
